...
class DBKudeatzailea:
    ...
    def taulakSortu(self):
        ...
        self.kurtsorea.execute("""CREATE TABLE IF NOT EXISTS `strava`.`bueltak` (
                                  ...
                                  `abiaduraMax` FLOAT NULL,
                                  `bbAbiadura` FLOAT NULL,
                                  PRIMARY KEY (`bueltaZenb`, `entrenaId` ),
    ...
    def datuakKargatu(self):
        ...
        while (i<entrenaLuzera) :
            ...
            #Ekipamendua bilatu
            if ("gear_id" in entrenamendua):
                ekipamenduId = entrenamendua["gear_id"]
            else:
                ekipamenduId= "";
            ...
            if emaitza != None :    #Entrenamendua aurkitzen badu -> UPDATE
                self.kurtsorea.execute("UPDATE entrenamendua SET izena=%s, mugimenduDenb= %s, mota=%s, noiz=%s, abiaduraMax=%s, bbAbiadura=%s, distantzia=%s, erreKaloria=%s, mapa=%s, ekipamenduId=%s WHERE id=%s;", (izena,mugimenduDenb,mota,noiz,abiaduraMax,bbAbiadura,distantzia,erreKaloria,mapa,ekipamenduId,id))
                self.db.commit()
                print("Eguneratzen...")

            else:                   #Entrenamendua aurkitzen EZ badu -> INSERT
                self.kurtsorea.execute("INSERT IGNORE INTO entrenamendua VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s );",(id,izena,mugimenduDenb,mota,noiz,abiaduraMax,bbAbiadura,distantzia,erreKaloria,mapa, ekipamenduId))
                self.db.commit()
                print("Sortzen...")
    ...
    def ekipamenduaBete(self, entrenamendua):
        if entrenamendua["gear_id"]!=None:
            ...
            #SELECT bat konprobatzeko ea badagoen ekipatu
            self.db.commit()
